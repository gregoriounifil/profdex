...
// Atributos de combate (sabor da planilha):
//  rigor = Ataque · didatica = Defesa · raciocinio = Velocidade/Precisão/Esquiva
export const STAT = {
  RIGOR: 'rigor',
  DIDATICA: 'didatica',
...
export const EFFECT = {
  ...
  IGNORE_DEFENSE: 'ignoreDefense', // ignora a Didática do alvo
...
export const MOVES_BY_TYPE = {
  logica: [
    ...
    { id: 'verdade-absoluta', name: 'Verdade Absoluta', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Didática', description: 'Axioma: ponto de partida que não se questiona; blinda a defesa do professor.', effects: [buff(STAT.DIDATICA)] },
    { id: 'cai-na-real', name: 'Cai na Real', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Rigor alvo', description: 'Tabela-verdade: expõe onde o argumento do oponente é falso e enfraquece seu ataque.', effects: [debuff(STAT.RIGOR)] },
  ...
  calculo: [
    ...
    { id: 'taxa-de-sofrimento', name: 'Taxa de Sofrimento', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Didática alvo', description: 'Derivada: mede a taxa de variação e corrói a defesa do alvo ponto a ponto.', effects: [debuff(STAT.DIDATICA)] },
    { id: 'ponto-de-maximo', name: 'Ponto de Máximo', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Rigor', description: 'Otimização: acha o máximo da função — e do próprio ataque.', effects: [buff(STAT.RIGOR)] },
  ...
  'ia-ml': [
    ...
    { id: 'puxao-de-sinapse', name: 'Puxão de Sinapse', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Rigor/turno', description: 'Rede neural: camadas que treinam e aumentam o ataque a cada turno.', effects: [growPerTurn(STAT.RIGOR)] },
    ...
    { id: 'corrige-na-marra', name: 'Corrige na Marra', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Rigor alvo', description: 'Backpropagation: propaga o erro de volta e reduz o ataque do alvo.', effects: [debuff(STAT.RIGOR)] },
    ...
    { id: 'desliga-uns-neuronios', name: 'Desliga uns Neurônios', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Raciocínio alvo', description: 'Dropout (regularização): desativa parte do processamento do adversário.', effects: [debuff(STAT.RACIOCINIO)] },
  ...
  robotica: [
    ...
    { id: 'radar-ligado', name: 'Radar Ligado', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Raciocínio', description: 'Sensores de proximidade: detectam o oponente antes e aumentam a velocidade.', effects: [buff(STAT.RACIOCINIO)] },
    { id: 'casco-de-lata', name: 'Casco de Lata', category: CATEGORY.DEFESA, power: null, accuracy: 1, raw: '+Didática 3t', description: 'Blindagem: o chassi reforçado eleva a defesa por 3 turnos.', effects: [buff(STAT.DIDATICA, 2, 3)] },
  ...
  arquitetura: [
    ...
    { id: 'fura-fila-do-bus', name: 'Fura-Fila do Bus', category: CATEGORY.ATAQUE, power: 70, accuracy: ACC.PADRAO, raw: '70 · ignora defesa', description: 'Barramento de dados: envia direto pelo bus e ignora parte da Didática.', effects: [ignoreDefense()] },
    ...
    { id: 'turbina-o-clock', name: 'Turbina o Clock', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Raciocínio', description: 'Overclock: eleva a frequência do processador e a velocidade.', effects: [buff(STAT.RACIOCINIO)] },
    ...
    { id: 'furou-a-fila-irq', name: 'Furou a Fila (IRQ)', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Raciocínio alvo', description: 'Interrupção (IRQ): rouba a prioridade de execução do adversário.', effects: [debuff(STAT.RACIOCINIO)] },
  ...
  npi: [
    ...
    { id: 'puxao-de-orelha', name: 'Puxão de Orelha', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Raciocínio alvo', description: 'Feedback duro do instrutor: a cobrança direta desconcentra e atrasa o adversário.', effects: [debuff(STAT.RACIOCINIO)] },
    { id: 'revisao-de-codigo', name: 'Revisão de Código', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Rigor alvo', description: 'Code review: aponta cada falha e antipadrão no código do oponente, enfraquecendo-o.', effects: [debuff(STAT.RIGOR)] },
    { id: 'apadrinha-ai', name: 'Apadrinha Aí', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Rigor/turno', description: 'Apadrinhamento: um veterano orienta o aluno, que fica mais forte a cada turno.', effects: [growPerTurn(STAT.RIGOR)] },
    { id: 'programacao-em-dupla', name: 'Programação em Dupla', category: CATEGORY.DEFESA, power: null, accuracy: 1, raw: '+Didática', description: 'Pair programming: dois na mesma tela, um revisa e o outro digita — a defesa dobra.', effects: [buff(STAT.DIDATICA)] },
  ...
  redes: [
    ...
    { id: 'aperto-de-mao', name: 'Aperto de Mão (3 vias)', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Rigor/precisão', description: 'Three-way handshake do TCP: estabelece conexão estável e dá bônus de ataque.', effects: [buff(STAT.RIGOR), buff(STAT.RACIOCINIO)] },
    { id: 'divide-pra-aguentar', name: 'Divide pra Aguentar', category: CATEGORY.DEFESA, power: null, accuracy: 1, raw: '+Didática', description: 'Balanceamento de carga: distribui o tráfego (e o dano) entre servidores.', effects: [buff(STAT.DIDATICA)] },
    { id: 'deu-lag', name: 'Deu Lag', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Raciocínio alvo', description: 'Latência de rede: o atraso deixa o adversário mais lento.', effects: [debuff(STAT.RACIOCINIO)] },
  ...
  banco: [
    ...
    { id: 'atalho-do-index', name: 'Atalho do Index', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Raciocínio', description: 'Índice: acelera drasticamente a busca e aumenta a velocidade.', effects: [buff(STAT.RACIOCINIO)] },
  ...
  algoritmos: [
    ...
    { id: 'anota-pra-nao-repetir', name: 'Anota pra Não Repetir', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Rigor acumula', description: 'Programação dinâmica (memoização): guarda subproblemas e vai ficando mais forte.', effects: [buff(STAT.RIGOR)] },
    { id: 'empilha-bonito', name: 'Empilha Bonito', category: CATEGORY.BUFF, power: null, accuracy: 1, raw: '+Didática', description: 'Estrutura de heap: organiza os dados em árvore e reforça a defesa.', effects: [buff(STAT.DIDATICA)] },
    { id: 'ficou-lerdo', name: 'Ficou Lerdo', category: CATEGORY.DEBUFF, power: null, accuracy: 1, raw: '-Raciocínio alvo', description: 'Complexidade O(n²): custo quadrático arrasta a velocidade do adversário.', effects: [debuff(STAT.RACIOCINIO)] },
